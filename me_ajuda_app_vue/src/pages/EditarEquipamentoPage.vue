...
// Objeto reativo para agrupar os campos do formulário
const form = ref({
  nome: '',
  descricao: '',
  preco: '',
...
// Recupera os dados da lista de equipamentos quando a tela é montada (substitui o useEffect)
onMounted(() => {
  ...
  if (stateEquipamento) {
    ...
    // Se não encontrar os dados, alerta e volta para a listagem
    $q.notify({
      type: 'warning',
  ...
  try {
    const res = await fetch(`http://localhost:8000/equipamentos/api/${equipamentoId.value}/`, {
      ...
      body: JSON.stringify({
        ...
        preco: parseFloat(form.value.preco), // Transforma para número/float
